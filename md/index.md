...
id：题目编号。每道题目都有唯一的编号，形如“jsi-数据集-数字”。demo和test分别代表示例集和测试集。
...
```json
{
    ...
    interpretation: “没入”与“水上”不能搭配。“没入”指在水面之下，即水中。
    answer: 错误
}
```
...
id：题目编号。每道题目都有唯一的编号，形如“rsr-数据集-数字”。demo和test分别代表示例集和测试集。
...
id：题目编号。每道题目都有唯一的编号，形如“rse-数据集-数字”。demo和test分别代表示例集和测试集。
...
```json
{
    ...
    text1: 今晚的月亮不耀眼，一副迷离恍惚的神气。桑桑看不清蒋一轮与白雀，但又分明看得清他们的影子。蒋一轮倚在柳树下，用的是让桑桑最着迷的姿势：两腿微微交叉着。白雀的动作在这样的月光笼罩下，显得格外的柔和。
    text2: 今晚的月亮不耀眼，一副迷离恍惚的神气。桑桑看不清蒋一轮与白雀，但又分明看得清他们的影子。蒋一轮倚在柳树边，用的是让桑桑最着迷的姿势：两腿微微交叉着。白雀的动作在这样的月光笼罩下，显得格外的柔和。
    interpretation: 两个文本的形式差异部分是“蒋一轮倚在柳树下”和“蒋一轮倚在柳树边”。在“蒋一轮倚在柳树下”中，蒋一轮高度低于柳树，从常识可以知道他位于柳树的附近；在“蒋一轮倚在柳树边”中，蒋一轮位于柳树的附近位置。因此，两个句子都可以表示蒋一轮位于柳树的附近位置的空间场景，text1和text2描述的空间场景相同。
    answer: 相同
}
```
...
id：题目编号。每道题目都有唯一的编号，形如“spr-zh-数据集-数字”。train, dev, test分别代表训练集, 验证集和测试集。
...
id：题目编号。每道题目都有唯一的编号，形如“spr-en-数据集-数字”。train, dev, test分别代表训练集, 验证集和测试集。
...
三、提交结果文件规定为jsonl格式，每行一个json对象，每个json对象至少包括id字段和answer字段。
